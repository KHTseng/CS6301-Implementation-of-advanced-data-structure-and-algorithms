...
public:
    ...
    /*void deleteVertex(Vertex v){////remove all edge about it
        for (list<Edge>::iterator it = v.Adj.begin(); it != v.Adj.end(); it++) {
            deleteEdge(*it);
        }
        for (list<Edge>::iterator it = v.revAdj.begin(); it != v.revAdj.end(); it++) {
            deleteEdge(*it);
        }
    }
    void deleteEdge(Edge e){////remove edge from two vertex's adj list
        e.Head.Adj.remove(e);
        e.Tail.Adj.remove(e);
        e.Tail.revAdj.remove(e);
    }
    bool checkDAG(){
        for (vector<Vertex>::iterator it = verts.begin(); it != verts.end(); it++) {//check undirected
            for (list<Edge>::iterator it_e = it->Adj.begin(); it_e != it->Adj.end(); it_e++) {
                if (it_e->direct == 0) {
                    return 0;
                }
            }
        ...
        for (vector<Vertex>::iterator it = verts.begin(); it != verts.end(); it++) {
            if (it->revAdj.empty()) {//no coming edge
